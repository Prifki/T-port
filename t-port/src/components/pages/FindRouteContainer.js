...
class FindRouteContainer extends Component {
  ...
  // Founds the shortest way from A to B
  findARoute = () => {
    ...
    if (validA && validB) {
      ...
      let timeData = this.countTransferTime(foundRoute);
  ...
  // Autocomplete for stop A and stop B
  stopAAutoComplete = (e) => {
    if (e.target.value) {
      let foundEntities = [];
      for (let stop in this.props.data.stops){
...
// ===========    DIJKSTRA    ===========
  ...
  tracePath = (table, start, end) => {
    ...
    route = this.handlePath(route, changes);
    return route;
  }

  handlePath = (route, changes) => {
    let newPath = [];
  ...
  countTransferTime = (route) => {
    ...
    if (route.length > 4) {
      for (let i = 1; i < route.length-3; i += 2) {
        for (let each in changeTimes) {
          if ((route[i][0] === changeTimes[each][1]) && (route[i+2][0] === changeTimes[each][2]) && (route[i+1][0] === changeTimes[each][0])) {
            timeData.push(changeTimes[each][3]);
          }
        }
      }
    }
